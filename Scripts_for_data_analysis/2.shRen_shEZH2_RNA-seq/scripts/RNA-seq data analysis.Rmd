---
title: "Scripts for RNA-seq data analysis"
author: "Haibo Liu"
date: "5/19/2022"
output: 
  html_document:
...
## 0. Summary of RNA-seq data analysis
Twenty one pairs of fastq files are located in the directory ***data/***, as described below. Analyses from raw data quality control to count table generation were conducted in a high performance computing cluster (HPC) with a LSF job management system. Further analyses were performed using the R programming language in a laptop computer running the Windows operation system.

|Group|Treatment|Replicate|R1 filename|R2 filename |
|-----|---------|---------|-----------|------------|
|Ezh2TP| shEZh2 + TP |1|Ezh2C1_1.fq.gz| Ezh2C1_2.fq.gz |
|Ezh2TP| shEZh2 + TP |2|Ezh2C2_1.fq.gz| Ezh2C2_2.fq.gz |
|Ezh2TP| shEZh2 + TP |3|Ezh2C3_1.fq.gz| Ezh2C3_2.fq.gz |
|Ezh2TP| shEZh2 + TP |4|Ezh2C4_1.fq.gz| Ezh2C4_2.fq.gz |
|Ezh2TP| shEZh2 + TP |5|Ezh2C5_1.fq.gz| Ezh2C5_2.fq.gz |
|Ezh2TP| shEZh2 + TP |6|Ezh2C6_1.fq.gz| Ezh2C6_2.fq.gz |
|Ezh2V| shEZh2 + Vehicle|1|EZH2V1_1.fq.gz| EZH2V1_2.fq.gz |
|Ezh2V| shEZh2 + Vehicle|2|EZH2V2_1.fq.gz| EZH2V2_2.fq.gz |
|Ezh2V| shEZh2 + Vehicle|3|EZH2V3_1.fq.gz| EZH2V3_2.fq.gz |
|Ezh2V| shEZh2 + Vehicle|4|EZH2V4_1.fq.gz| EZH2V4_2.fq.gz |
|Ezh2V| shEZh2 + Vehicle|5|EZH2V5_1.fq.gz| EZH2V5_2.fq.gz |
|RenC| shRen + TP|1|RenC1_1.fq.gz | RenC1_2.fq.gz |
|RenC| shRen + TP|2|RenC2_1.fq.gz | RenC2_2.fq.gz |
|RenC| shRen + TP|3|RenC3_1.fq.gz |  RenC3_2.fq.gz |
|RenC| shRen + TP|4|RenC5_1.fq.gz |  RenC5_2.fq.gz |
|RenC| shRen + TP|5|RenC6_1.fq.gz |  RenC6_2.fq.gz |
|RenV| shRen + Vehicle|1|RenV1_1.fq.gz | RenV1_2.fq.gz |
|Renv| shRen + Vehicle|2|RenV2_1.fq.gz | RenV2_2.fq.gz |
|RenV| shRen + Vehicle|3|RenV3_1.fq.gz | RenV3_2.fq.gz |
|RenV| shRen + Vehicle|4|RenV7_1.fq.gz | RenV7_2.fq.gz |
|RenV| shRen + Vehicle|5|RenV8_1.fq.gz | RenV8_2.fq.gz |
...
in=data
fastq=(`ls $in/*.gz`)
out=results/001.shEZh2.fastqc.out
mkdir -p $out
fastqc -o $out  -t 4 --extract ${fastq[$i]}
```

## 2. Build the genome index using STAR
...
STAR --runMode genomeGenerate \
        ...
        --outFileNamePrefix ${overhang} 
```

## 3. Alignment of paired-end reads using STAR

```{sh alignment, eval = FALSE}
#!/bin/sh
...
rm -rf   ${out}/${name[$i]}_STARtmp ${out}/${name[$i]}_STARgenome \
         ${out}/${name[$i]}Log.out  ${out}/${name[$i]}Log.progress.out \
         ${out}/${name[$i]}SJ.out.tab
```

## 4. Count matrix generation using featureCounts
...
featureCounts  -p  -d 25 -D 1000 -s 0  -F GTF -g gene_id \
               -G $fasta  -t exon  -T 8   -a $gtf \
               -o $out/${count_table}.PE  ${bam[@]}
               
cut -f 2-6 --complement $out/${count_table}.PE  | \
perl -p -e 's/^#.+$//; s{results/003.STAR.out/(.+?)Aligned.out.bam}{$1}g' > \
           $out/${count_table}.PE.formatted.txt
```
...
WriteXLS(x =de_genes_exp_senescence, 
         ExcelFileName = 
           paste0("Table 5.DESeq.differential.expressed.Fridman.and.Tasdemir.",                     "senescence.genes.expression.value4heatmap.xlsx"),
         row.names = TRUE, 
